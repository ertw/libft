...
static int		find_line(const int fd, char **line, char *full_line, int delim)
{
	static t_tuple	b;

	while (1)
	{
		if (b.str && (b.nl_ptr = ft_strchr(b.str + b.pos, delim)))
		{
...
int				get_delim(const int fd, char **line, int delim)
{
	char			*full_line;
	...
	return ((!line || fd < 0 || read(fd, full_line, 0) == READ_ERROR)
			? READ_ERROR
			: (find_line(fd, line, full_line, delim)));
}
